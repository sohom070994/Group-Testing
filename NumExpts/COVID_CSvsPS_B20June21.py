...
print('\n'+str(round(np.mean(a),5))+'('+str(round(1.96*np.std(a)/np.size(a),5))+')')
#%% 3. Debugging Block
# abc=get_risk_vector()
# print(abc)
# print(len(abc))
   ...
   for w2 in list(range(0,1,1)):
       if(w1+w2<=10):
           ...
           results = results.append({'w1':lambda_1,
                                     ...
                                     'CI_OFCS8':ConfInt(of_cs8),
                                     'OF_CS5':round(np.mean(of_cs5),4),
                                     'CI_OFCS5':ConfInt(of_cs5)},ignore_index=True)
print("Time:")
...
# print(time.time() - start_time) 
#%% 8. OUTPUT TO CSV
results.to_csv('Covid_CSvsPS_B20_200simsJune21.csv')      
#%% 7. SIMULATIONS
lambda_1=0
lambda_2=0
simulations = 100

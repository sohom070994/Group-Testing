...
def get_risk_vector():
   ...
   #   risk_vec[i]=round(coin_flip()/100,4)
    ...
    a=np.random.choice(elements, 100, p=probabilities)
    return a
...
#%% 7. TEST_SIMULATIONS
a=np.zeros(simulations)
for sim in tqdm(range(0,simulations),desc='sim loop',position=0,leave=True):
    risk_vec=get_risk_vector()
    a[sim]=np.mean(risk_vec)
#%% 7. SIMULATIONS
# sys.stdout = open("FINALRESULTS.txt", "w")
start_time=time.time()
results = pd.DataFrame(columns= ['w1',
...
for w1 in tqdm(list(range(0, 11, 1)),desc='lambda loop',position=0,leave=True):
    for w2 in list(range(0,1,1)):
        if(w1+w2<=10):
            lambda_1=w1/10
            lambda_2=w2/10
            ...
            results = results.append({'w1':lambda_1,
                                      'w2':lambda_2,
                                      'OF_D':round(np.mean(of_d),10),
                                      'CI_OFD':1.96*np.std(of_d)/np.size(of_d),
                                      'OF_CS5':round(np.mean(of_cs5),4),
                                      'CI_OFCS5':1.96*np.std(of_cs5)/np.size(of_cs5),
                                      'OF_CS8':round(np.mean(of_cs8),4),
                                      'CI_OFCS8':1.96*np.std(of_cs8)/np.size(of_cs8)},ignore_index=True)
print("Time:")
print(time.time() - start_time) 
    #%% 8. OUTPUT TO CSV
results.to_csv('Covid_CSvsPS_200.csv')      
#%% 7. SIMULATIONS
lambda_1=0
lambda_2=0
simulations = 100
...
of_d=np.empty(simulations)
of_cs=np.empty([pop,simulations])


for sim in tqdm(range(0,simulations),desc='sim loop',position=0,leave=True):
    risk_vec=get_risk_vector()
    for i in range(pop):
        current_node_list=list(range(0,100,i+1))
        current_node_list.reverse()
        of_cs[i,sim]=calc_measof(current_node_list) 
    #Optimal Scenario
    risk_vec.sort()

...
def weight_ND(Node_1,Node_2):
  weight = (lambda_1*Exp_FalseNeg_ND_Mod(Node_1,Node_2))+(lambda_2*Exp_FalsePos_ND_Mod(Node_1,Node_2))+((1-lambda_1-lambda_2)*Exp_Test_Num_ND(Node_1,Node_2))
  return weight
...
for w1 in tqdm(list(range(0, 12, 2))+[9.6],desc='lambda loop',position=0,leave=True):
   for w2 in list(range(0,12,2))+[0.2]:
       # if(w1!=9.6)&(w2==0.2):
       #     continue
       # if(w1==9.6)&(w2!=0.2):
       #     continue
...
results.to_csv('Results50sims2.csv')

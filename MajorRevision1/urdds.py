...
#     elements = [i*7 for i in elements]
...
def DorfmanSim(rv,part):
    ...
    for grp in part:
        if len(grp)==1:
            ...
            # print(grp,len(grp),k,sens)
            exp_tests+=int(np.random.choice([1,1+len(grp)],1,p=[1-sens,sens]))
    return exp_tests
#%%
# print(Dorfmansim)
...
ET_URDDS=np.empty(simulations)
opti=np.empty(simulations)
for sim in tqdm(range(0,simulations),position=0, leave=True):
    uns_risk_vec=get_risk_vector()
    risk_vec=np.sort(uns_risk_vec)
...
#%% 7.a SIMULATIONS- Normal RV
# ET_CP=np.empty(simulations)
ET_URDDS=np.empty(simulations)
for sim in tqdm(range(0,simulations)):
    uns_risk_vec=get_risk_vector()
    risk_vec=np.sort(uns_risk_vec)
    perm_vec=np.argsort(uns_risk_vec)
    # cp_part=list(range(0,pop,5))
    # cp_part.reverse()
    # ET_CPDG=np.empty(DSim)
    # for dg in range(0,DSim):
    ...
    for n, p in (enumerate(partition(list(range(0,pop))),1)):
        ...
        ET=np.empty(DSim)
        for dg in range(0,DSim):
            ET[dg]=DorfmanSim(uns_risk_vec,p)
        ET_allpart[n-1]=round(np.mean(ET),4) 
        # if (urdds_part_mod==p):
        #     print('URDDS_OF: '+str(round(np.mean(ET),4) ))
        print(p,str(round(np.mean(ET),4))
    print('---')    
